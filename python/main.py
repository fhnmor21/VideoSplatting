...
def parse_args() -> argparse.Namespace:
    ...
    # ------------------------------------------------------------------ #
    p.add_argument(
        "-o", "--output", type=Path, default=Path("./gs_pipeline_output"),
        help="Root output directory (frames/, colmap/, gaussian/ placed inside)",
    )
    p.add_argument(
        "--gs-repo", type=Path, default=Path("./gaussian-splatting"),
        help="Path to the cloned gaussian-splatting repository",
    )
    p.add_argument(
        "--conda-env", default="gaussian_splatting",
    ...
    stages.add_argument("--skip-extract",    action="store_true", help="Skip frame extraction")
    stages.add_argument("--skip-colmap",     action="store_true", help="Skip COLMAP reconstruction")
    stages.add_argument("--skip-training",   action="store_true", help="Skip GS training")
    stages.add_argument("--skip-render",     action="store_true", help="Skip eval render + metrics")
    ...
    gs.add_argument(
        "--iterations", type=int, default=30000, metavar="N",
        help="Total training iterations",
    )
    gs.add_argument(
        "--densify-start", type=int, default=500, metavar="N",
        help="Iteration to start Gaussian densification",
    )
    gs.add_argument(
        "--densify-end", type=int, default=15000, metavar="N",
        help="Iteration to stop Gaussian densification",
    )
    gs.add_argument(
        "--densify-grad-threshold", type=float, default=0.0002, metavar="T",
        help="Gradient threshold for densification. Lower = more Gaussians.",
    )
    gs.add_argument(
        "--resolution-cap", type=int, default=1600, metavar="PX",
        help="Cap longest image side to this many pixels before training "
             "(VRAM guard for large phone images)",
    )
    gs.add_argument(
        "--test-holdout", type=int, default=8, metavar="N",
        help="Hold out every Nth image as a test/eval split",
    )
    gs.add_argument(
        "--checkpoint-interval", type=int, default=7000, metavar="N",
        help="Save a checkpoint every N iterations (0 = only at end)",
    )
    gs.add_argument(
        "--viewer-port", type=int, default=6009, metavar="PORT",
