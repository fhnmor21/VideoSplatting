...
from pipeline.utils import (
    CommandError, count_ply_points, find_conda_sh,
    format_duration, log_header, log_info, log_success, log_warn,
    run, run_in_conda,
)
...
class GaussianTrainer:
    ...
    def run(self) -> bool:
        ...
        image_count = len(list((source / "images").glob("*.jpg"))) + \
                      len(list((source / "images").glob("*.png")))
    ...
    def _build_train_cmd(self) -> List:
        ...
        cmd: List = [
            "python", str(cfg.train_script),

            # I/O
            "--source_path", str(cfg.colmap_dense),
            "--model_path",  str(cfg.gs_output),

            # Iterations
            "--iterations", str(cfg.iterations),

            # Densification — stop at half-way for interiors (avoids wall over-splat)
            "--densify_from_iter",      str(cfg.densify_start),
            "--densify_until_iter",     str(cfg.densify_end),
            ...
            "--images", str(cfg.colmap_dense / "images"),

            # Evaluation split
            "--eval",
            "--llffhold", str(cfg.test_holdout),
    ...
    def _render(self) -> None:
        ...
        cmd: List = [
            "python", str(self.cfg.render_script),
            "--model_path",  str(self.cfg.gs_output),
            "--source_path", str(self.cfg.colmap_dense),
            "--iteration",   str(self.cfg.iterations),
            "--skip_train",
        ]
        ...
        try:
            if self._conda_sh:
                run_in_conda(
                    self._conda_sh, self.cfg.conda_env, cmd,
                    dry_run=self.cfg.dry_run, cwd=self.cfg.gs_repo,
                )
            else:
    ...
    def _metrics(self) -> None:
        ...
        cmd: List = [
            "python", str(self.cfg.metrics_script),
            "--model_path", str(self.cfg.gs_output),
            "--iteration",  str(self.cfg.iterations),
        ]
    ...
    def _print_summary(self, image_count: int) -> None:
        ...
        print(f"  COLMAP sparse: {self.cfg.best_sparse_model or self.cfg.colmap_sparse}")
